...
<section class="hero">
    ...
    <div class="hero-content">
        <span class="article-cat"
            style="background-color: rgba(0, 168, 107, 0.15); color: var(--color-accent); font-weight: 800; padding: 6px 16px; border-radius: 50px;">
            Filter Kategori
        </span>
        <h1 style="margin-top: 15px; margin-bottom: 10px;"><?= \App\Core\Request::escape($category['name']) ?></h1>
        <p style="margin-bottom: 0; opacity: 0.8;">
            <?= \App\Core\Request::escape($category['description'] ?? 'Eksplorasi artikel di bawah kategori ini.') ?>
        </p>
    </div>
</section>
...
<div class="main-layout">
    ...
    <div class="content-area">
        <?php if (empty($posts)): ?>
            <div
                style="background-color: var(--color-white); border: 1px solid rgba(18,18,18,0.05); padding: 40px; border-radius: 16px; text-align: center; color: var(--color-text-muted);">
                <p style="font-size: 16px; font-weight: 600;">Belum ada artikel dalam kategori ini.</p>
                <a href="<?= BASE_URL ?>"
                    style="color: var(--color-accent); font-weight: 700; text-decoration: none; display: inline-block; margin-top: 15px;">Kembali
                    ke Beranda</a>
            </div>
        <?php else: ?>
                        ...
                        <div class="classic-post-layout">
                            <?php if (!empty($post['image'])): ?>
                                <div class="classic-img-wrapper">
                                    <img src="<?= BASE_URL ?>/assets/uploads/<?= \App\Core\Request::escape($post['image']) ?>"
                                        alt="<?= \App\Core\Request::escape($post['title']) ?>">
                                </div>
                            <?php endif; ?>
            ...
            <?php if ($totalPages > 1): ?>
                <div class="pagination">
                    <a href="<?= BASE_URL ?>/category/<?= $category['slug'] ?>?page=<?= $currentPage - 1 ?>"
    ...
    <aside class="sidebar" style="height: auto; border: none; padding: 0; background: transparent;">
        ...
        <div class="widget">
            ...
            <form action="<?= BASE_URL ?>" method="GET" class="search-form"
                style="padding: 4px; box-shadow: none; border-color: rgba(18,18,18,0.08); background-color: var(--color-cream)">
                <input type="text" name="q" class="search-input" placeholder="Cari..."
                    style="color: var(--color-dark); padding: 8px 12px; font-size: 14px;">
                ...
                <?php foreach ($categories as $cat): ?>
                    <li>
                        <a href="<?= BASE_URL ?>/category/<?= \App\Core\Request::escape($cat['slug']) ?>"
                            ...
                            style="<?= ($cat['id'] === $category['id']) ? 'color: var(--color-accent);' : '' ?>">
                            <?= \App\Core\Request::escape($cat['name']) ?>
                            <span class="cat-count"
                                style="<?= ($cat['id'] === $category['id']) ? 'background-color: var(--color-accent); color: var(--color-dark);' : '' ?>"><?= $cat['post_count'] ?></span>
                        </a>
                    </li>
                <?php endforeach; ?>
            </ul>
